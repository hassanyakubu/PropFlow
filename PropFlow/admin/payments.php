...
$sql = "SELECT p.*, prop.title, u_tenant.full_name as tenant_name, u_landlord.full_name as landlord_name
        FROM payments p 
        ...
        JOIN users u_landlord ON prop.owner_id = u_landlord.user_id";

if ($property_filter !== 'all') {
    $sql .= " WHERE prop.property_id = '$property_filter'";
}

$sql .= " ORDER BY p.payment_date DESC";

// Fetch properties for filter
$props = $db->db_fetch_all("SELECT property_id, title FROM properties");
...
<head>
    ...
    <title>Global Payments - PropFlow Admin</title>
    ...
    <div class="flex flex-col md:flex-row flex-grow container mx-auto px-4 py-8 gap-8">
        ...
        <main class="flex-grow">
            <div class="flex justify-between items-center mb-8">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Global Payment Ledger</h1>
                    <p class="text-gray-500 dark:text-gray-400 mt-1">Platform-wide view of all rent payments.</p>
                </div>
            ...
            <div class="bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-xl shadow-sm overflow-hidden"
                ...
                <div class="print-header hidden text-center mb-8">
                    <h2 class="text-2xl font-bold">PropFlow Global Rent Ledger</h2>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                        <thead class="bg-gray-50 dark:bg-slate-900/50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Landlord</th>
                            ...
                            <?php if ($payments): ?>
                                <?php foreach ($payments as $pay): ?>
                                    <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                            <?php echo htmlspecialchars($pay['landlord_name']); ?>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                            ...
                                            <div class="text-xs font-mono"><?php echo substr($pay['transaction_reference'] ?: '', 0, 8); ?>...</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full <?php echo $pay['payment_status'] === 'paid' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400' : 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-400'; ?>">
